...
class UpsellfunnelsController extends Controller
{
     ...
     */	
	 ...
	 public function Savefunnel(Request $request){
		...
		$this->validate($request, [
            'name' => 'required|unique:upsellfunnels|max:100|min:4',
    ...
    public function StatusfunnelUpsells($id){
        ...
        $funnelupsell = Upsellfunnel_upsellproducts::where('funnelid', $id)->get()->first();
        if($funnelupsell!== null){
            //echo $exist->status;
        if($exist->status=='Disable'){
            ...
            return redirect('upsellfunnels')->with('error', 'Unable to change status for empty funnel.');  
        }   
