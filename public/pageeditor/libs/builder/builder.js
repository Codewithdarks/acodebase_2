...
if (WEO === undefined) var WEO = {};

WEO.defaultComponent = "_base";
WEO.preservePropertySections = true;
...
WEO.dragHtml = '<div style="background:limegreen;;width:100%;height:3px;border:1px solid limegreen;box-shadow:0px 0px 2px 1px rgba(0,0,0,0.14);"></div>';

WEO.baseUrl = document.currentScript ? document.currentScript.src.replace(/[^\/]*?\.js$/, '') : '';
WEO.imgBaseUrl = WEO.baseUrl;
...
WEO.Components = {
	...
	render: function (type) {
		...
		if (!(WEO.preservePropertySections && section.length)) {
			componentsPanelSections[defaultSection].html('').append(tmpl("WEO-input-sectioninput", { key: "default", header: component.name }));
		...
		if (component.beforeInit) component.beforeInit(WEO.Builder.selectedEl.get(0));
		...
		var fn = function (component, property) {
			return property.input.on('propertyChange', function (event, value, input) {
				...
				var element = WEO.Builder.selectedEl;
				...
				if (property.htmlAttr) {
					...
					if (property.htmlAttr == "class" && property.validValues) {
						...
						oldStyle = $("#WEOjs-styles", window.FrameDocument).html();
						element = WEO.StyleManager.setStyle(element, property.key, value);
					}
					else if (property.htmlAttr == "innerHTML") {
						element = WEO.ContentManager.setHtml(element, value);
					}
					else {
					...
					if (property.htmlAttr == "style") {
						mutation = {
							...
							oldValue: oldStyle,
							newValue: $("#WEOjs-styles", window.FrameDocument).html()
						};

						WEO.Undo.addMutation(mutation);
					} else {
						WEO.Undo.addMutation(
							{
								type: 'attributes',
								target: element.get(0),
								attributeName: property.htmlAttr,
				...
				if (!property.child || !property.parent) WEO.Builder.selectNode(element);

				return element;
			});
		};

		var nodeElement = WEO.Builder.selectedEl;
		...
		for (var i in component.properties) {
			...
			if (property.init) {
				...
				if (property.htmlAttr == "style") {
					...
					var value = WEO.StyleManager.getStyle(element, property.key);//getStyle returns declared style
				} else
					if (property.htmlAttr == "innerHTML") {
						var value = WEO.ContentManager.getHtml(element);
					} else {
			...
			if (property.inputtype == SectionInput) {
				...
				var row = $(tmpl('WEO-property', property));
		...
		if (component.init) component.init(WEO.Builder.selectedEl.get(0));
	}
};
...
WEO.WysiwygEditor = {
	...
	editorSetStyle: function (tag, style = {}, toggle = false) {
		let iframeWindow = WEO.Builder.iframe.contentWindow;
	...
	init: function (doc) {
		...
		$("#font-family").on("change", function (e) {
			...
			WEO.FontsManager.addFont(option.dataset.provider, this.value, element);
...
WEO.Builder = {
	...
	loadControlGroups: function () {
		...
		componentsList.each(function () {
			...
			for (group in WEO.ComponentsGroup) {
				...
				components = WEO.ComponentsGroup[group];

				for (i in components) {
					componentType = components[i];
					component = WEO.Components.get(componentType);
					...
					if (component) {
						...
						if (component.image) {
							...
							item.css({
								backgroundImage: "url(" + WEO.imgBaseUrl + component.image + ")",
	...
	loadSectionGroups: function () {
		...
		sectionsList.each(function () {
			...
			var list = $(this);
			var type = this.dataset.type;
			for (group in WEO.SectionsGroup) {
				list.append(
					`<li class="header" data-section="${group}"  data-search="">
				...
				sections = WEO.SectionsGroup[group];

				for (i in sections) {
					sectionType = sections[i];
					section = WEO.Sections.get(sectionType);
					...
					if (section) {
						...
						if (section.image) {
							...
							var image = ((section.image.indexOf('/') == -1) ? WEO.imgBaseUrl : '') + section.image;
	...
	loadBlockGroups: function () {
		...
		blocksList.each(function () {
			...
			for (group in WEO.BlocksGroup) {
				...
				blocks = WEO.BlocksGroup[group];

				for (i in blocks) {
					blockType = blocks[i];
					block = WEO.Blocks.get(blockType);
					...
					if (block) {
						...
						if (block.image) {
							...
							var image = ((block.image.indexOf('/') == -1) ? WEO.imgBaseUrl : '') + block.image;
	...
	loadUrl: function (url, callback) {
		...
		self.initCallback = callback;
		if (WEO.Builder.iframe.src != url) WEO.Builder.iframe.src = url;
	},
	...
	_loadIframe: function (url) {
		...
		return this.documentFrame.on("load", function () {
			...
			WEO.WysiwygEditor.init(window.FrameDocument);
			WEO.StyleManager.init(window.FrameDocument);
	...
	_frameLoaded: function () {
		...
		self.frameHead.append('<link data-WEO-helpers href="' + WEO.baseUrl + '../../css/WEOjs-editor-helpers.css" rel="stylesheet">');

		self._initHighlight();

		$(window).triggerHandler("WEO.iframe.loaded", self.frameDoc);
	...
	loadNodeComponent: function (node) {
		data = WEO.Components.matchNode(node);
		var component;

		if (data)
			component = data.type;
		else
			component = WEO.defaultComponent;

		WEO.component = WEO.Components.get(component);
		WEO.Components.render(component);

	},

	moveNodeUp: function (node) {
		if (!node) {
			node = WEO.Builder.selectedEl.get(0);
		}
	...
	selectNode: function (node) {
		...
		if (self.texteditEl && self.selectedEl.get(0) != node) {
			WEO.WysiwygEditor.destroy(self.texteditEl);
		...
		if (target) {
			...
			try {
				...
				WEO.Breadcrumb.loadBreadcrumb(target.get(0));
	...
	_initHighlight: function () {
		...
		self.frameHtml.on("mouseup dragend touchend", function (event) {
			...
			if (self.isDragging) {
				...
				WEO.Builder.highlightEnabled = true;
				if (self.iconDrag) self.iconDrag.remove();
				$("#component-clone").remove();

				if (self.dragMoveMutation === false) {
					if (self.component.dragHtml || WEO.dragHtml) { //if dragHtml is set for dragging then set real component html
				...
				if (self.dragMoveMutation === false) {
					...
					WEO.Undo.addMutation(self.dragMoveMutation);
		...
		self.frameHtml.on("dblclick", function (event) {
			...
			if (WEO.Builder.isPreview == false) {
				self.selectPadding = 10;
				self.texteditEl = target = $(event.target);

				WEO.WysiwygEditor.edit(self.texteditEl);
		...
		self.frameHtml.on("click", function (event) {
			...
			if (WEO.Builder.isPreview == false) {
				if (event.target) {
					...
					if (WEO.component.resizable) {
						$("#select-box").addClass("resizable");
						self.resizeMode = WEO.component.resizeMode;
					} else {
	...
	_initBox: function () {
		...
		$(".components-list li ol li", addSectionBox).on("click", function (event) {
			var html = WEO.Components.get(this.dataset.type).html;
		...
		$(".blocks-list li ol li", addSectionBox).on("click", function (event) {
			var html = WEO.Blocks.get(this.dataset.type).html;
		...
		$(".sections-list li ol li", addSectionBox).on("click", function (event) {
			var html = WEO.Sections.get(this.dataset.type).html;
	...
	_initDragdrop: function () {
		...
		$('.drag-elements-sidepane ul > li > ol > li[data-drag-type]').on("mousedown touchstart", function (event) {
			...
			if ($this.data("drag-type") == "component") {
				self.component = WEO.Components.get($this.data("type"));
			}
			else if ($this.data("drag-type") == "section") {
				self.component = WEO.Sections.get($this.data("type"));
			}
			else if ($this.data("drag-type") == "block") {
				self.component = WEO.Blocks.get($this.data("type"));
			}

			if (self.component.dragHtml) {
				html = self.component.dragHtml;
			} else if (WEO.dragHtml) {
				html = WEO.dragHtml;
			} else {
	...
	removeHelpers: function (html, keepHelperAttributes = false) {
		...
		html = html.replace(/<[^>]+?data-WEO-helpers.+?>/gi, "");
		//attributes
		if (!keepHelperAttributes) {
			html = html.replace(/\s*data-WEO-\w+(=["'].*?["'])?\s*/gi, "");
		}
	...
	getHtml: function (keepHelperAttributes = true) {
		...
		$(window).triggerHandler("WEO.getHtml.before", doc);
		...
		WEO.FontsManager.cleanUnusedFonts();

		$(window).triggerHandler("WEO.getHtml.after", doc);

		var filter = $(window).triggerHandler("WEO.getHtml.filter", html);
	...
	saveAjax: function (fileName, startTemplateUrl, callback, saveUrl) {
		var data = {};
		data["file"] = (fileName && fileName != "") ? fileName : WEO.FileManager.getCurrentFileName();
...
WEO.CodeEditor = {
	...
	init: function (doc) {
		$("#WEO-code-editor textarea").val(WEO.Builder.getHtml());

		$("#WEO-code-editor textarea").keyup(function () {
			delay(() => WEO.Builder.setHtml(this.value), 1000);
		});

		//load code on document changes
		WEO.Builder.frameBody.on("WEO.undo.add WEO.undo.restore", function (e) { WEO.CodeEditor.setValue(); });
		//load code when a new url is loaded
		WEO.Builder.documentFrame.on("load", function (e) { WEO.CodeEditor.setValue(); });
...
WEO.Gui = {
	...
	init: function () {
		$("[data-WEO-action]").each(function () {
			on = "click";
			if (this.dataset.WEOOn) on = this.dataset.WEOOn;

			$(this).on(on, WEO.Gui[this.dataset.weoAction]);
			if (this.dataset.WEOShortcut) {
				$(document).bind('keydown', this.dataset.WEOShortcut, WEO.Gui[this.dataset.weoAction]);
				$(window.FrameDocument, window.FrameWindow).bind('keydown', this.dataset.WEOShortcut, WEO.Gui[this.dataset.weoAction]);
			}
		});
	},

	undo: function () {
		if (WEO.WysiwygEditor.isActive) {
			WEO.WysiwygEditor.undo();
		} else {
			WEO.Undo.undo();
		}
		WEO.Builder.selectNode();
	},

	redo: function () {
		if (WEO.WysiwygEditor.isActive) {
			WEO.WysiwygEditor.redo();
		} else {
			WEO.Undo.redo();
		}
		WEO.Builder.selectNode();
	...
	save: function () {
		$('#textarea-modal textarea').val(WEO.Builder.getHtml());
	...
	saveAjax: function () {
		...
		var saveUrl = this.dataset.WEOUrl;
		var url = WEO.FileManager.getPageData('file');

		return WEO.Builder.saveAjax(url, null, function (data) {
	...
	download: function () {
		filename = /[^\/]+$/.exec(WEO.Builder.iframe.src)[0];
		uriContent = "data:application/octet-stream," + encodeURIComponent(WEO.Builder.getHtml());
	...
	toggleEditor: function () {
		$("#WEO-builder").toggleClass("bottom-panel-expand");
		$("#toggleEditorJsExecute").toggle();
		WEO.CodeEditor.toggle();
	},

	toggleEditorJsExecute: function () {
		WEO.Builder.runJsOnSetHtml = this.checked;
	},

	preview: function () {
		(WEO.Builder.isPreview == true) ? WEO.Builder.isPreview = false : WEO.Builder.isPreview = true;
		$("#iframe-layer").toggle();
		$("#WEO-builder").toggleClass("preview");
	},

	openPage: function () {
		$("#filemanager .tree").slideToggle();
	},
	...
	newPage: function () {
		...
		newPageModal.modal("show").find("form").off("submit").submit(function (e) {
			...
			WEO.FileManager.addPage(data.name, data);
			e.preventDefault();

			return WEO.Builder.saveAjax(data.file, data.startTemplateUrl, function (data) {
				WEO.FileManager.loadPage(name);
				WEO.FileManager.scrollBottom();
	...
	setDesignerMode: function () {
		...
		WEO.Builder.setDesignerMode(designerMode);
	},
	//layout
	...
	toggleFileManager: function () {
		WEO.Gui.togglePanel("#filemanager", "--builder-filemanager-height");
	},

	toggleLeftColumn: function () {
		WEO.Gui.togglePanel("#left-panel", "--builder-left-panel-width");
	},

	toggleRightColumn: function (rightColumnEnabled = null) {
		rightColumnEnabled = WEO.Gui.togglePanel("#right-panel", "--builder-right-panel-width");

		$("#WEO-builder").toggleClass("no-right-panel");
		$(".component-properties-tab").toggle();

		WEO.Components.componentPropertiesElement = (rightColumnEnabled ? "#right-panel" : "#left-panel") + " .component-properties";
	...
	init: function (doc) {
		if (doc) {
			...
			for (let i = 0; i < doc.styleSheets.length; i++) {
				...
				if (_style.ownerNode.id && _style.ownerNode.id == "WEOjs-styles") {
					style = _style;
					break;
				}
			}
			...
			if (!style) {
				this.cssContainer = $('<style id="WEOjs-styles"></style>');
			...
			return this.cssContainer = $("#WEOjs-styles", doc);
		}
	},
	...
	getSelectorForElement: function (element) {
		...
		while (currentElement.parentElement) {
			...
			let classSelector = Array.from(currentElement.classList).map(function (className) {
				if (WEO.Builder.ignoreClasses.indexOf(className) == -1) {
					return "." + className;
				}
			}).join("");
		...
		if (node.hasChildNodes()) {
			for (var j = 0; j < node.childNodes.length; j++) {
				...
				if (child && child["attributes"] != undefined &&
					(matchChild = WEO.Components.matchNode(child))) {
		...
		for (i in tree) {
			...
			if (tree[i].children.length > 0) {
				var li = $('<li data-component="' + node.name + '">\
								<label for="id' + id + '" style="background-image:url(' + WEO.imgBaseUrl + node.image + ')"><span>' + node.name + '</span></label>\
...
WEO.SectionList = {
	...
	init: function (allowedComponents = {}) {
		...
		$(this.selector).on("click", "> div .controls", function (e) {
			...
			if (node) {
				...
				delay(
					() => WEO.Builder.frameHtml.animate({
						scrollTop: $(node).offset().top - ($(node).height() / 2)
					}, 500),
					300);

				//node.click();
				WEO.Builder.selectNode(node);
				WEO.Builder.loadNodeComponent(node);
			}
		...
		$(this.selector).on("click", ".up-btn", function (e) {
			var section = $(e.currentTarget).parents(".section-item");
			var node = section.data("node");
			WEO.Builder.moveNodeUp(node);
			WEO.Builder.moveNodeUp(section.get(0));
		...
		$(this.selector).on("click", ".down-btn", function (e) {
			var section = $(e.currentTarget).parents(".section-item");
			var node = section.data("node");
			WEO.Builder.moveNodeDown(node);
			WEO.Builder.moveNodeDown(section.get(0));
		...
		*/
		...
		$("#sections .sections-list").on("click", " .add-section-btn", function (e) {
			var section = WEO.Sections.get(this.parentNode.dataset.type);
			console.log("section: ", section)
			var node = $(section.html);
			// if(section.type === "bootstrap4/product-section"){
			// 	$(WEO.Builder.frameBody).find('#exampleModal').modal("show")
			// }
			var sectionType = node[0].tagName.toLowerCase();
			var afterSection = $(sectionType + ":last", WEO.Builder.frameBody);

			if (afterSection.length) {
				afterSection.after(node);
			} else {
				if (sectionType != "footer") {
					afterSection = $("footer:first", WEO.Builder.frameBody);

					if (afterSection.length) {
						afterSection.before(node);
					} else {
						$(WEO.Builder.frameBody).append(node);
					}
				} else {
					$(WEO.Builder.frameBody).append(node);
				}
	...
	addSection: function (data) {
		var section = $(tmpl("WEO-section", data));
...
WEO.FileManager = {
	...
	init: function (allowedComponents = {}) {
		...
		$(this.tree).on("click", "li[data-page] label", function (e) {
			...
			let title = ""
			if (page) {
				// title = $(this.parentNode).data("title");
				// console.log("title: ", title)
				// let modalHtml =
				// 	`
				// 		<div class="modal fade modal-full" id="PageNameModal" tabindex="-1" role="dialog" aria-labelledby="PageNameModalLabel" aria-hidden="true">
				// 		<div class="modal-dialog modal-md modal-dialog-scrollable" role="document">
				// 			<div class="modal-content">
				// 			<div class="modal-header">
				// 				<h5 class="modal-title" id="PageNameModalLabel">New page</h5>
								
				// 				<button type="button" class="btn btn-sm close_pageNameModal" data-bs-dismiss="modal" aria-label="Close">
				...
				// 							<div class="data">
				// 								<label class="mb-2">Enter page name</label>
				...
				// 					<button type="button" class="btn btn-lg btn-secondary close_pageNameModal" data-bs-dismiss="modal">Close</button>
				// 					<button type="button" class="btn btn-lg btn-primary save-btn" data-bs-dismiss="modal">Create</button>
				// 				</div>
				// 			</div>
				// 			</div>
				// 		</div>
				...
				// 	$("#PageNameModal .save-btn").on("click", () => {
				// 		$('#PageNameModal').remove()
						WEO.FileManager.loadPage(page, allowedComponents);
					// });
					// $("#PageNameModal .close_pageNameModal").on("click", () => {
					// 	$('#PageNameModal').remove()
					// });
				// }
	...
	addPage: function (name, data) {
		...
		if (data.folder) {
			if (!(folder = this.tree.find('li[data-folder="' + data.folder + '"]')).length) {
				...
				folder = $(tmpl("WEO-filemanager-folder", data));
		...
		folder.append(
			tmpl("WEO-filemanager-page", data));
	},
	...
	addComponent: function (name, url, title, page) {
		$("[data-page='" + page + "'] > ol", this.tree).append(
			tmpl("WEO-filemanager-component", { name: name, url: url, title: title }));
	},
	...
	loadPage: function (name, allowedComponents = false, disableCache = true) {
		...
		WEO.Builder.loadUrl(url + (disableCache ? (url.indexOf('?') > -1 ? '&r=' : '?r=') + Math.random() : ''),
			function () {
				WEO.FileManager.loadComponents(allowedComponents);
				WEO.SectionList.loadSections(allowedComponents);
				WEO.StyleManager.init();
			});
			$("#filemanager .tree").slideUp();
	},
...
WEO.Breadcrumb = {
	...
	addElement: function (data, element) {
		var li = $(tmpl("WEO-breadcrumb-navigaton-item", data));
	...
	loadBreadcrumb: function (element) {
		...
		while (currentElement.parentElement) {
			this.addElement({
				"name": WEO.Builder._getElementType(currentElement).toLowerCase(),
			}, currentElement);
