GoogleFontsManager = {
	...
	updateFontList: function () {
		let googleFontsLink = $("#google-fonts-link", WEO.Builder.frameHead);
		...
		if (googleFontsLink.length < 1) {
			...
			WEO.Builder.frameHead.append(googleFontsLink);
		}
...
WEO.FontsManager.addProvider("google", GoogleFontsManager);
	
let googleFonts = {};
let googlefontNames = [];
//load google fonts list and update wyswyg font selector and style tab font-family list
$.ajax({
	url: WEO.baseUrl + "../../resources/google-fonts.json",
	cache: true,
	dataType: "json",
	success: function(data) {
		...
		let list = WEO.Components.getProperty("_base", "font-family");
		list.onChange = function (node,value, input, component) {
			let option = input.options[input.selectedIndex];
			WEO.FontsManager.addFont(option.dataset.provider, value, node);
			return node;
		};
		...
		WEO.Components.updateProperty("_base", "font-family", {data:list.data});
	}
});
