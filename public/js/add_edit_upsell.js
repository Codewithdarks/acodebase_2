$(document).ready(function () { 
    ...
    if(isupsells>=1){
        $('.updownsellgroup_1 > .upsell1 > .list-item-button > ul > li > a.edit-btn').addClass('aaaaaaa');
             ...
             $.ajax({
                ...
                success: function (response) {
                   // console.log(response);    
                    ...
                    $('.preview-btn').attr('data-id',currentid).attr('data-type',type).removeClass('disabled');
                    $('#savefunnelupdosell').removeClass('disabled');
                    if(extobj.discounttype=='Percentage'){
                        ...
                        $('.discountprice').html('Sale:<strong>'+app.Currency.formatMoney(parseFloat(discountedprice*100), app.moneyFormat)+'</strong>');
    ...
    $(document).on('click', '.view-btn, .preview-btn', function () {
        ...
        window.open(""+CHECKOUTDOMAIN+"previewfunnel?id="+updnsellid+"&type="+type+"", '_blank');
        }
    });
    ...
    $(document).on('keyup','#productname', function() { 
        ...
        if (term.length >= 3){
            $.ajax({
                ...
                crossDomain:true,
                url: SHOPIFYMAINDOMAIN+'/search/suggest.json?q='+term+'&resources[type]=product&limit=10&resources[options][unavailable_products]=hide&resources[options][fields]=title,product_type,variants.title,tag',
    ...
    $(document).on('click','.search-dropdown ul li', function() {
       ...
       //<p>'+productdesc+'</p>
        var productdetail ='<div class="product-image"><img src="'+productimage+'" alt="'+producttitle+'"></div><div class="product-content"><div class="product-desc"><h3>'+producttitle+'</h3></div><div class="mainprice product-price" data-price="'+productprice+'">Price: <strong>'+app.Currency.formatMoney(parseFloat(productprice*100), app.moneyFormat)+'</strong></div><div class="discountprice product-price"></div></div>';
    ...
    $(document).on('blur','#discountamount', function() {
        ...
        if(typeof discounttype !== "undefined"){
            if(discounttype=='Percentage'){
                if(discount>=100){
                    if(!$(".preview-btn").hasClass("disabled")){$(".preview-btn").addClass("disabled");}
                    if(!$("#savefunnelupdosell").hasClass("disabled")){$("#savefunnelupdosell").addClass("disabled");}
                    alert("You can't give 100% discount on upsell.");    
                }else{
                    $("#savefunnelupdosell").removeClass("disabled");
                    $(".preview-btn").removeClass("disabled");
                    var discountget = (productprice * discount) / 100;
                ...
                $('.discountprice').html('Sale:<strong>'+app.Currency.formatMoney(parseFloat(discountedprice*100), app.moneyFormat)+'</strong>');
                    ...
                    if(!$(".preview-btn").hasClass("disabled")){$(".preview-btn").addClass("disabled");}
                    if(!$("#savefunnelupdosell").hasClass("disabled")){$("#savefunnelupdosell").addClass("disabled");}
                    alert("You can't give discount more than product price on upsell.");  
                }
