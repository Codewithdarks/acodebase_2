...
<head>
    ...
    <link rel="icon" href="@if((new \App\Http\Controllers\SettingsController)->RetrieveValue('favicon') !='') {{ (new \App\Http\Controllers\SettingsController)->RetrieveValue('favicon') }}@else{{ url('images/fav.png') }}@endif">
    ...
    <main>
        <div class="container">
            ...
            @if(Session::has('error'))
                <div class="row justify-content-center nofify-box">
                    <div class="col-4"><div class="alert alert-danger">{{ Session::get('error') }}</div></div>
                </div>
            @endif
        </div>
        @yield('content')
